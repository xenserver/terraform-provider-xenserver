	...
	if !data.RawVdiPath.IsNull() {
		...
		if err != nil {
			...
			if vhdSize, vhdErr := GetVHDOriginalSize(filePath); vhdErr == nil {
				...
				// Given Go SDK use int for virtual size, we have to check for potential integer overflow before conversion
				const maxInt = int(^uint(0) >> 1)
				if vhdSize > uint64(maxInt) {
					resp.Diagnostics.AddError(
						"VHD size too large",
						fmt.Sprintf("VHD file size (%d bytes) exceeds maximum supported size (%d bytes)", vhdSize, maxInt),
					)
					return
				}
